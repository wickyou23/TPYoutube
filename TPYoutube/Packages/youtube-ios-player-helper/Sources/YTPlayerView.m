...
- (void)notifyDelegateOfYouTubeCallbackUrl:(NSURL *) url {
  ...
  if (query) {
      NSArray *paramStrings = [query componentsSeparatedByString:@"&"];
      for (NSString *item in paramStrings) {
          NSArray *paramDetails = [item componentsSeparatedByString:@"="];
          if (paramDetails.count == 2) {
              [params setValue:paramDetails[1] forKey:paramDetails[0]];
          }
      }
  }
  ...
  if ([action isEqual:kYTPlayerCallbackOnReady]) {
    if (self.initialLoadingView) {
      ...
      if ([self.delegate respondsToSelector:@selector(playerViewDidBecomeReady:andPlayDuration:)]) {
        float duration = -1;
        if (params.count != 0 && [params valueForKey:@"duration"]) {
            duration = [params[@"duration"] floatValue];
        }
        
      [self.delegate playerViewDidBecomeReady:self andPlayDuration:duration];
    }
...
- (void)changeDocumentTitle:(NSString* _Nonnull)title {
    [self.webView evaluateJavaScript:[NSString stringWithFormat:@"document.title = \"%@\";", title]
                   completionHandler:nil];
}
